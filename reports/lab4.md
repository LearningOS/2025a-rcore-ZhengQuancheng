...
- easy-fs中，root inode起着什么作用？如果root inode中的内容损坏了，会发生什么？
    - `ROOT_INODE` 是整个文件系统的根目录, 所有文件访问都必须从 `ROOT_INODE` 开始. 
    - 若 `ROOT_INODE` 损坏, 无法通过文件名访问对应的文件, 文件数据本身可能完好.

- 举出使用 pipe 的一个实际应用的例子
    - 过滤进程: `ps aux | grep "vscode"`
    - 统计文件行数: `cat README.md | wc -l`

- 一个易用的多进程通信机制
    - 采用发布-订阅模型, 即所有用户进程都连接到内核代理服务进程, 用户进程之间不再直接通信. 用户进程可订阅特定的主题, 当某个进程向内核代理服务进程发布某个主题消息时, 内核代理服务进程负责将该消息分发给所有订阅该主题的用户进程.
